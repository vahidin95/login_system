...
if (isset($_POST['submit'])) {
	...
	$allow = array('jpg', 'jpeg',); // which file extencion we want to allow to user upload;
	if (in_array($fileActualExt , $allow)) { // in_array() function with first param inside chech if it exist in array which we put inside like another param.
		if ($fileError === 0) {
			if ($fileSize < 1000000) {
				...
				header("Location: ../home.php?upload=errorbig");
			}
		}else{
			header("Location: ../home.php?upload=error_");
		}
	}else{
		header("Location: ../home.php?upload=typeerror");
	}
}else{
